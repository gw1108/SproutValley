...
**Project layout for sub-agents:** this is a Godot 4 project; game source (`.gd`, `.tscn`, resources) lives under `sprout-valley/`, design docs under `design/`, and tunable numbers in `data/balance.csv` (read via `BalanceData.get_value`). Tell agents NOT to scan `sprout-valley/addons/`, `sprout-valley/reports/`, `SourceArt/`, any `ARCHIVE` folder, or `thoughts/` outside `thoughts/shared/`, unless the question explicitly targets them.
...
## Code References
- `sprout-valley/path/to/file.gd:123` — description
- `sprout-valley/path/to/scene.tscn` — description
...
Replace `[FULL_FILE_PATH]` with the absolute path printed by `create_thought.py`. Do not summarize findings, list assumptions, list open questions, or add follow-up instructions — assumptions and open questions belong inside the document itself, and the user can run `/iterate-research-codebase [research-doc-path]` if they want follow-ups.
